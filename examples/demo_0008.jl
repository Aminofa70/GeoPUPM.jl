...
using Comodo
using PUPM
using Rotations  # For RotX, RotY, RotZ
...
# --- Remesh the surface ---
n1 = 5000  # target number of surface points
...
function create_bc(dh, grid)
    ...
    add!(ch, dbc)
    dbc = Dirichlet(:u, getfacetset(grid, "top_2"), (x, t) -> [0.0, 0.0, 0.0], [1, 2, 3])
    add!(ch, dbc)
    Ferrite.close!(ch)
    return ch
end

# --- Build DOF handler and constraints ---
dh = create_dofhandler(grid)
ch = create_bc(dh, grid)

# --- Write VTK file with boundary constraints ---
...
println("✅ VTK boundary condition file written to: ", output_dir)
